...
// Quick sanity tests to check that import for a parameterized package works. This uses Go as the language choice shouldn't matter for the test.
// Regression test for https://github.com/pulumi/pulumi/issues/17289.
//
//nolint:paralleltest // pulumi new is not parallel safe
func TestImportParameterizedSmoke(t *testing.T) {
	t.Skip("Skipping until #17289 is done")
	...
	e.RunCommand("pulumi", "login", "--cloud-url", e.LocalURL())
	e.RunCommand("pulumi", "new", "go", "--yes")

	// Install the terraform-provider plugin
	e.RunCommand("pulumi", "plugin", "install", "resource", "terraform-provider")

	// Add the terraform random provider to the project
	_, _ = e.RunCommand("pulumi", "package", "add", "terraform-provider", "hashicorp/random")

	// TODO: Ideally "package add" would do this step for us, but for now manually fixup the go.mod
	goModPath := filepath.Join(projectDir, "go.mod")
	goMod, err := os.ReadFile(goModPath)
	require.NoError(t, err)
	goMod = append(goMod, []byte("\nreplace github.com/pulumi/pulumi-random => ./sdks/random\n")...)
	...
	// choose the right Provider, because "random" alone isn't enough to know to use the paramterized provider
	...
	assert.Contains(t, stack, "\"parameterization\": ")
}
